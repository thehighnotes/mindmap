...
/**
 * Detect if file is old format based on extension and content
 */
function isOldFormatFile(filename, data) {
    // Check file extension
    const isOldExtension = filename.endsWith('.mindmap');
    
    // Check data structure (old format has nextNodeId)
    const hasOldStructure = data.hasOwnProperty('nextNodeId');
    
    // Check for old ID format in nodes
    const hasOldIds = data.nodes?.some(node => 
        /^node-\d+$/.test(node.id)
    );
    
    return isOldExtension || hasOldStructure || hasOldIds;
}

/**
 * Convert old format to new format
 */
function convertToNewFormat(data, filename) {
    const converted = {
        version: '2.0.0',
        format: 'mindmap2',
        metadata: {
    ...
    // Migrate nodes with new IDs
    const idMap = new Map();
    
    if (data.nodes) {
        converted.nodes = data.nodes.map(node => {
            const newId = IDGenerator.isOldFormat(node.id) 
                ? IDGenerator.migrateOldId(node.id, 'node')
                : node.id;
            ...
            return {
                ...node,
                id: newId,
                // Add any new properties for v2 format
    ...
    // Migrate connections with updated references
    if (data.connections) {
        converted.connections = data.connections.map(conn => {
            const newId = conn.id && IDGenerator.isOldFormat(conn.id)
                ? IDGenerator.generateConnectionId()
                : conn.id || IDGenerator.generateConnectionId();
    ...
    delete converted.nextNodeId;  // No longer needed!
...
// Helper function to save mindmap with specific filename
function saveMindmapAs(data, filename) {
...
function exportToJson() {
    ...
    const data = {
        ...
        // NOTE: No nextNodeId - we don't need it anymore!
        
        // Enhanced metadata
        metadata: {
            version: projectInfo.version || '2.0.0',
    ...
    // Default to .mindmap2 extension
    const filename = (getMindmapTitle ? getMindmapTitle() : 'mindmap') + '.mindmap2';
    ...
    showToast('Mindmap opgeslagen als ' + filename);
}
...
function importFromJson(file) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Check if old format
            if (isOldFormatFile(filename, data)) {
                // Prompt user for conversion
                const message = `Dit bestand lijkt in het oude Mindmap formaat (v1) te zijn.\n\n` +
                    `Oude formaat gedetecteerd:\n` +
                    `- Bestand: ${filename}\n` +
                    `- Heeft oude IDs: ${data.nodes?.some(n => /^node-\d+$/.test(n.id)) ? 'Ja' : 'Nee'}\n` +
                    `- Heeft nextNodeId: ${data.hasOwnProperty('nextNodeId') ? 'Ja' : 'Nee'}\n\n` +
                    `Wilt u het converteren naar het nieuwe Mindmap2 formaat?\n` +
                    `Dit zal:\n` +
                    `• Unieke IDs genereren die niet conflicteren\n` +
                    `• ID-conflictproblemen verwijderen\n` +
                    `• Het bestand compatibel maken met toekomstige functies\n\n` +
                    `(U wordt gevraagd het geconverteerde bestand op te slaan)`;
                
                if (confirm(message)) {
                    // Convert to new format
                    const converted = convertToNewFormat(data, filename);
                    ...
                    // Prompt to save as new format
                    setTimeout(() => {
                        if (confirm('Conversie succesvol! Wilt u het bestand nu opslaan in het nieuwe formaat?')) {
                            // Change extension to .mindmap2
                            const newFilename = filename.replace(/\.mindmap$/, '.mindmap2');
                            saveMindmapAs(converted, newFilename);
                        }
                    }, 100);
                    
                    showToast('Bestand succesvol geconverteerd naar Mindmap2 formaat!');
                } else {
                    // Load without conversion (compatibility mode)
                    showToast('Laden in compatibiliteitsmodus. Sommige functies kunnen beperkt zijn.', true);
                ...
                // Already new format or user's choice, load directly
                // Check if this is an enhanced project file or legacy format
                if (data.metadata && data.metadata.formatVersion === '2.0' && data.versionHistory) {
                    // New enhanced format with embedded version history
                    importEnhancedProject(data);
...
function loadMindmapData(data) {
    try {
        ...
        // Laad de gegevens
        // No longer need nextNodeId - using ID generator now
