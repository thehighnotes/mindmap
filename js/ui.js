...
function setupEventListeners() {
    ...
    // Enhanced wheel handler for zoom and trackpad panning
    canvasContainer.addEventListener('wheel', function(e) {
        e.preventDefault();
        
        // Detect if this is a trackpad pan gesture (2-finger drag)
        // On trackpads, when panning with 2 fingers:
        // - deltaX and deltaY represent pan distances
        // - ctrlKey is false (unless pinch-zooming)
        // - deltaMode is usually 0 (pixels)
        
        const isTrackpadPan = !e.ctrlKey && e.deltaMode === 0 && 
                              (Math.abs(e.deltaX) > 0 || Math.abs(e.deltaY) > 0);
        
        if (isTrackpadPan && Math.abs(e.deltaX) > Math.abs(e.deltaY) * 0.3) {
            // This is likely a 2-finger pan gesture (has significant X movement)
            // Pan the canvas
            ...
            console.log('🖐️ Trackpad pan:', { deltaX: e.deltaX, deltaY: e.deltaY });
            return;
        }
        
        // Otherwise, treat as zoom (scroll wheel or pinch-to-zoom)
        console.log('🖱️ Wheel zoom event:', { deltaY: e.deltaY, zoomLevel, ctrlKey: e.ctrlKey });
        ...
        // For pinch-to-zoom on trackpad (ctrlKey + wheel)
        let zoomDelta;
        if (e.ctrlKey) {
        ...
        const newZoom = Math.max(0.1, Math.min(3, zoomLevel * zoomDelta));
        
        console.log('🔍 Zoom change:', { oldZoom: zoomLevel, delta: zoomDelta, newZoom });
